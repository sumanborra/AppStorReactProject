...
class AppStore extends Component {
  state = {searchInput: '', appsShown: tabsList[0].tabId}
  changeApps = event => {
  ...
  changeTab = tabId => {
    const {appsShown} = this.state
    const tabDisplay = tabsList.filter(eachItem => eachItem.tabId === tabId)

    const updateTabDisplay = tabDisplay[0]
    this.setState({appsShown: updateTabDisplay.tabId})
    //console.log(tabsList[updateTabDisplay]);
  }
  render() {
    const {searchInput, appsShown} = this.state
    const searchResults = appsList.filter(
      eachItem => eachItem.category === appsShown,
    )
    const appsUpdated = searchResults.filter(eachItem =>
      eachItem.appName.toLowerCase().includes(searchInput.toLocaleLowerCase()),
    )
    console.log(appsShown)
    return (
      <div className="background-container">
        ...
        <ul className="tab-items-list-container">
          {tabsList.map(eachItem => (
            <TabItem
              tabsList={eachItem}
              key={eachItem.tabId}
              changeTab={this.changeTab}
              isActive={eachItem.tabId === appsShown}
            />
          ))}
        </ul>

        <ul className="list-style-appslist">
          {appsUpdated.map(eachItem => (
